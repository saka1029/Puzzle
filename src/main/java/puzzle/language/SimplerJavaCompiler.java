...
/**
 * -d オプションを使って、コンパイル結果をクラスファイルとして保存する方法。
 * クラスローダを定義する必要がない。FileManagerも必要ない。
 */
public class SimplerJavaCompiler {
     ...
     * 指定したディレクトリ(destination)からクラスオブジェクトをロードするためのClassLoaderを返します。
     * <br>
     * [呼び出し例]
     * <pre><code>
     ...
     * </code></pre>
     * @param destination クラスファイルの出力先ディレクトリを指定します。
     * @param options コンパイルオプションを指定します。省略する場合はnullを指定できます。
     * @param sources コンパイルするクラス名とソーステキストを指定します。
     * @return コンパイル結果のクラスオブジェクトをロードするためのクラスローダを返します。
     * <code>ClassLoader loader = compile(...); Class<?> clazz = loader.loadClass(クラス名);</code>
     * @throws SimplerJavaCompileError コンパイルエラーが発生したときにスローします。
     * @throws MalformedURLException destinationで指定したディレクトリをURLに変換できないときにスローします。
     */
    public static ClassLoader compile(File destination, List<String> options, Source... sources)
            throws SimplerJavaCompileError, MalformedURLException {
        ...
        CompilationTask task = compiler.getTask(null, null, diagnostics, compilationOptions, null, List.of(sources));
        if (!task.call())
